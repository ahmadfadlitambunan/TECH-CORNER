...
$list = mysqli_query($conn, "SELECT * FROM posting WHERE kategori='Laptop / Notebook' ORDER BY tanggal_posting DESC LIMIT $awal, $jumlah_data_perhalaman");
            ...
            <div class="col-md-8">
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination justify-content-center">
                        <li class="page-item">
                            <?php if ($activepage > 1) : ?>
                                <a class="page-link" href="laptop.php?halaman=<?= $activepage - 1 ?>" aria-label="Previous">
                        ...
                        <?php for ($i = 1; $i <= $jumlahpage; $i++) : ?>
                            <?php if ($i === $activepage) : ?>
                                <li class="page-item  "><a class="page-link" href="laptop.php?halaman=<?= $i ?>"><?= $i ?></a></li>
                            <?php else : ?>
                                <li class="page-item "><a class="page-link" href="laptop.php?halaman=<?= $i ?>"><?= $i ?></a></li>
                            <?php endif; ?>
                        <?php endfor; ?>
                        <li class="page-item">
                            <?php if ($activepage < $jumlahpage) : ?>
                                <a class="page-link" href="laptop.php?halaman=<?= $activepage + 1 ?>" aria-label="Next">
